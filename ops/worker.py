...
def upload_blob_parallel(bucket_name, source_file_name, job):
    ...
    thread = threading.Thread(target=upload_blob, args=(bucket_name, source_file_name, destination_blob_name, job))
    thread.start()
...
while not q.empty():
    ...
    if item is not None:
        ...
        process = Popen(['surge', '-P', '-T', '-B1,2,3,4,5,7,9', '-t0', '-f0', '-S',  mf], stdout=PIPE, stderr=PIPE)
        ...
        for line in iter(process.stdout.readline, b''):
            if currentIterCount < 10000000:
                ...
                currentIterCount += 1
                ofile = mf+"_"+str(i)
                zf = zipfile.ZipFile(ofile+".zip", mode="w", compression=zipfile.ZIP_DEFLATED)
        ...
        if process.returncode != 0: 
            ...
            q._db.set(job + ':' + mf, getJobLog(end - start, 'None', 'None', start, end, stdout, stderr))
            q._db.lpush(job + ':' + 'failed', mf)
            q.complete(item)
        else:
            end =  getTimeStamp()
            q._db.set(job + ':' + mf, getJobLog(end - start,  totalCompressedFileSize, totalStructuresCount, start, end, stdout, stderr))
